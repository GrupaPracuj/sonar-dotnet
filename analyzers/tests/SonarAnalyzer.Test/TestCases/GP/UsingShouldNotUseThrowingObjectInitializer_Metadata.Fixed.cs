namespace Tests.Diagnostics
{
    ...
    public class MetadataInitializerMembers
    {
        ...
        {
            using var resource = new Library.InitOnlyDisposable { Value = Compute() }; // Fixed
        }

        public void RequiredFromMetadataHasNoFix()
        {
            using var resource = new Library.RequiredDisposable { Value = Compute() }; // Fixed
        }

        public void RequiredFieldFromMetadataHasNoFix()
        {
            using var resource = new Library.RequiredFieldDisposable { Value = Compute() }; // Fixed
        }
        ...
        {
            using var resource = new Library.PlainDisposable(); // Fixed
            resource.Value = Compute();
        }
